...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ error: error.message || 'Failed to fetch live dispatches' }, {  status: 500 , headers: { 'Cache-Control': 'public, s-maxage=60, stale-while-revalidate=300' } });
  }
}
